...
`dvim` starts the Dev Container for the current workspace and opens Neovim in the
corresponding directory. `dsh` opens an interactive shell in the same location.
...
`dvim` and `dsh` use the Git root as the Dev Container workspace. Outside a Git
repository, they use the current directory. Run them from the workspace or one of
its subdirectories.
